...
# We will add the best child to the fringe, in order of priority
...
D = []
# The evaluation function (including the heuristics of tour length and the distance to our goal node)
E = []

# THE ALGORITHM IS ENHANCED HERE - the first node is defined as a city
# The node identifier (how many steps we have taken from our spot)
newID = 0
# Setting the initial state of the first node
S.append(1)
...
tour.append([1])
# Setting the path-cost of the root node to zero
tour_length.append(0)
# Setting the depth of the root node to 0
D.append(1)
# Setting the evaluation function of the root node to zero. We will store the result of our evaluation function here
E.append(0)
...
F = [(newID, S[0], P[0], tour[0], tour_length[0], D[0], E[0])]
...
# It will not be empty when supplied data, since we have defined an empty start node
    ...
    while F:
        ...
        parent_id = F[-1][2]
        # The result of the evaluation function on the cities visited so far
        ...
        # We will sort the fringe, according to the size of the smallest evaluation function, each iteration
        F.sort(key=lambda tup: tup[6], reverse=True)
        ...
        for child in child_city:
            ...
            # Calculate the result of our evaluation function
            # Add the result of our two heuristics (our tour length and the distance to our goal node)
            # If we are on our starting city, or the empty node, set the evaluation to an upper bound of a large number
            if len(tour_calc) <= 1:
                evaluation_calc = 999999999999999999999999
            else:
                first_node = previous_tour[0]
                evaluation_calc = tour_length[newID] + dist_matrix[child - 1][first_node - 1]
            E.append(evaluation_calc)
            ...
            if tour_length[newID] < current_tour_solution_length: # ADDED
                if tour[newID] == None:
                    F.append((newID, S[newID], P[newID], tour[newID], tour_length[newID], D[newID], E[newID]))
                else:
